from src.domain.Validators import BookValidatorException, ClientValidatorException


def add_book_test(book_service):
    x = len(book_service.get_all_books())
    book_service.add_book('   title ', '   author   ')

    assert len(book_service.get_all_books()) == x+1
    exists, b_id = False, ''
...
def add_client_test(client_service):
    x=len(client_service.get_all_clients())
    client_service.add_client('  name   ')
    assert len(client_service.get_all_clients()) == x+1
    exists, c_id = False, ''
