...
from src.services.UndoRedo import  UndoManager, RedoManager
...
class Menu:
    ...
    def ui_add(self):
        if self.__current_manager == 'clients':
            name=input('Name:')
            self.__client_service.add_client(name.strip())
    ...
    def run_console(self):
        ...
        com_dict = {'s': self.ui_switch_manager, 'a': self.ui_add, 'd': self.ui_display, 'r': self.ui_remove,
                    ...
                    'mra': self.ui_most_rented_author,'undo':self.ui_undo,'redo':self.ui_redo}
        lc=None
        while True:
            self.ui_menu()
            ...
            if command == 'x':
                ...
                #if command=='redo' and lc!='undo':
                   # raise Exception("redo follows undo")
                try:
                    if command=='undo':
                        lc='undo'
                    if command == 'redo' : #and lc != 'undo':
                        if lc!='undo': #and lc!='undo' and lc!='d' and lc!='mra' and lc!='mrb' and lc!='mac' and lc!='s' and lc!=None :
                            raise Exception("redo follows undo")
                    else:
                        if command!='d' and command!='mra' and command!='mrb' and command!='mac' and command!='s':
                            lc=command
