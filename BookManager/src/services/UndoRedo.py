...
def add_rental_handler(repo, rental,b_repo):
    repo.delete_rental(rental.id)
    b = b_repo.find_book_book_id(rental.book_id)
    b.is_available = True
    RedoManager.register_undo(repo, RedoHandler.ADD_RENTAL, rental,b_repo)
...
class UndoManager:
    ...
    def register_operation(target_object, handler, *args):
        ...
        #UndoManager.__last_operation='a'
    @staticmethod
    def undo():
        ...
        #UndoManager.__undo_operation='undo'
    @staticmethod
    def clear_undoes():
...
class UndoManager:
    ...
    def register_operation(target_object, handler, *args):
        UndoManager.__undo_operations.append(UndoOperation(target_object, handler, args))
        #UndoManager.__last_operation = 'undo'  # Set last operation as undo
    ...
    def undo():
        ...
        undo_operation = UndoManager.__undo_operations.pop()
        undo_operation.handler(undo_operation.target_object, *undo_operation.args)
        UndoManager.__last_operation = 'redo'  # Set last operation as redo after undo
...
def add_rental_handler_redo(repo, rental,b_repo):
    repo.save_rental(rental)
    b=b_repo.find_book_book_id(rental.book_id)
    b.is_available=False
    UndoManager.register_operation(repo, UndoHandler.ADD_RENTAL, rental,b_repo)
...
class RedoManager:
    ...
    @staticmethod
    def redo():
        if UndoManager.__last_operation != 'undo':  # Check if the last operation was not an undo
            raise ValueError("Cannot redo without a prior undo operation")
        ...
        redo = RedoManager.__redo_operations.pop()
        redo.handler(redo.target, *redo.args)
        RedoManager.__last_operation = 'redo'  # Set last operation as redo after successful redo
    """
    @staticmethod
    def clear_redoes():
