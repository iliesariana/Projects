...
from src.services.UndoRedo import UndoHandler,UndoManager
...
class BookService:
    ...
    def add_book(self, title, author):
        ...
        :return:-
        """
        val=self.__create_book_id()
        b = Book(val, ' '.join(title.title().split()), ' '.join(author.title().split()), True)
    ...
    def remove_book(self, book_id):
        ...
        b = self.find_books(book_id)[0]
        self.__bo_repo.delete_book_book_id(book_id)
        UndoManager.register_operation(self.__bo_repo,UndoHandler.REMOVE_BOOK,b)
